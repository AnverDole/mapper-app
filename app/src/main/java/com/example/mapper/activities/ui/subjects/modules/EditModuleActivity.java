...
public class EditModuleActivity extends AppCompatActivity {
    ...
    public static int SUCCESS_RESULT_CODE = 103;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        subjectId = getIntent().getExtras().getInt("subject_id");
        moduleId = getIntent().getExtras().getInt("module_id");
        moduleName = getIntent().getExtras().getString("module_name");
        priority = getIntent().getExtras().getInt("module_priority");
        moduleDuration = getIntent().getExtras().getString("module_duration");

        String hours = moduleDuration.split(":")[0];
        String minutes = moduleDuration.split(":")[1];
        binding.moduleDurationTxt.setText(String.format("%s:%s", hours, minutes));

        Integer subjectNo = getIntent().getExtras().getInt("subject_no");
        getSupportActionBar().setTitle(getResources().getString(R.string.activity_edit_modules_title) + " #" + subjectNo);

        binding.moduleTitleTxt.setText(moduleName);
        ...
        binding.prioritySpinner.setSelection(priority - 1);
    ...
    private void saveData(){
        if (validateFields()) { // all fields are valid
            ...
            Log.e("SS", String.valueOf(priority));

            Mapper.updateModule(this, subjectId, moduleId, data, new Mapper.TaskTransition() {
                @Override
